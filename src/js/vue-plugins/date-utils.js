...
export default function install(app) {
  // $language is looked up lazily (not captured at install time), because plugin install
  // order matters: this plugin is installed before the gettext-plugin one.
  ...
  const dateUtils = {
    ...
    timeAgo(arg) {
      return dayjs(arg).locale(locales[currentLang()]).fromNow();
    },
    ...
    toLocalizedString(arg, formatString) {
      ...
      return dayjs(arg).locale(locales[currentLang()]).format(formatString);
    },

    toTechnicalString(arg) {
      return dayjs(arg).locale(locales[currentLang()]).format('YYYY-MM-DD HH:mm:ss');
    },

    month(monthNumber) {
      return dayjs().locale(locales[currentLang()]).localeData().months()[monthNumber];
    },
