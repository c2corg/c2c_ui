...
export default function install(app) {
  ...
  // because plugin install order matters (this plugin is installed before gettext/user).
  const globals = app.config.globalProperties;

  const documentUtils = {
    getCreationTitle(documentType) {
      if (documentType === 'outing') return globals.$gettext('add an outing');
      if (documentType === 'route') return globals.$gettext('add a route');
      if (documentType === 'image') return globals.$gettext('add an image');
      if (documentType === 'waypoint') return globals.$gettext('add a waypoint');
      if (documentType === 'map') return globals.$gettext('add a map');
      if (documentType === 'xreport') return globals.$gettext('add an incident/accident report');
      if (documentType === 'area') return globals.$gettext('add an area');
      if (documentType === 'book') return globals.$gettext('add a book');
      if (documentType === 'article') return globals.$gettext('add an article');

      throw new Error(`Unexpected type : ${documentType}`);
    },
    ...
    getDocumentTitle(document, lang) {
      ...
      const locale = this.getLocaleSmart(document, lang);
    ...
    getDocumentLicense(document) {
      const documentType = this.getDocumentType(document.type);
    ...
    getLocaleSmart(document, lang) {
      ...
      let result = lang ? this.getLocaleStupid(document, lang) : null;
      ...
      result = this.getLocaleStupid(document, globals.$user.lang);
      if (result) {
        return result;
      }
    ...
    getOutingDatesLocalized(document) {
      ...
      if (!date_start) {
        return globals.$gettext('Invalid date');
      }

      const start = globals.$dateUtils.parseDate(date_start);
      const end = globals.$dateUtils.parseDate(date_end);

      if (!globals.$dateUtils.isSameYear(start, end)) {
        return this.formatDate(start, 'll') + ' - ' + this.formatDate(end, 'll');
      } else if (!globals.$dateUtils.isSameMonth(start, end)) {
        return this.formatDate(start, 'D MMMM') + ' - ' + this.formatDate(end, 'll');
      } else if (!globals.$dateUtils.isSameDay(start, end)) {
    ...
    formatDate(arg, formatString) {
      return globals.$dateUtils.toLocalizedString(arg, formatString);
    },
    ...
    lengthWithUnit(length) {
      ...
      if (length > 1000) {
        ...
        return `${l}&nbsp;${globals.$gettext('kilometers')}`;
      }
      return `${length}&nbsp;${globals.$gettext('meters')}`;
    },
    ...
    heightDiffUpWithUnit(hdu) {
      ...
      return `${hdu}&nbsp;${globals.$gettext('meters')}`;
    },
  };

  app.config.globalProperties.$documentUtils = documentUtils;
}
